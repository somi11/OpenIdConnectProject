...
namespace Cmos.IDP.Services
{
    public class LocalUserService : ILocalUserService
    {
          ...
          string password)
        {
            ...
            var verifcationResult = _passwordHasher.VerifyHashedPassword(user, user.Password, password);

            return (verifcationResult == PasswordVerificationResult.Success);
        }
        ...
        public void AddUser(User userToAdd , string password)
        {
            ...
            {
                throw new Exception("User Must be Unique");
            }
            //security code genertor
            userToAdd.SecurityCode = Convert.
                ToBase64String(RandomNumberGenerator.GetBytes(128));
            userToAdd.SecurityCodeExpirationDate = DateTime.UtcNow.AddMinutes(50);
        ...
        public async Task<bool> ActivateUserAsync(string securityCode)
        {
          ...
          var user = await _context.Users.FirstOrDefaultAsync(u => u.SecurityCode == securityCode
